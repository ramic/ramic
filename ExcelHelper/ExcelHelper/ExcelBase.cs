...
namespace ExcelBase
{
    ...
    public class ExcelBase
    {
        ...
        public static DataTable ImportExcel(string fileName, string sheetName, bool HasHeader)
        {
            DataTable table = ExecuteReader(fileName, "select * from [" + sheetName + "]", null);
        ...
        /// <param name="HasHeader">是否存在表头。若是会把工作表第一行转成DataTable的列名</param> 
        /// <returns>导入后的DataTable</returns> 
        public static DataTable ImportExcel(string fileName, bool HasHeader)
        {
            return ImportExcel(fileName, "sheet1$", HasHeader);
        }
